...
jQuery.extend(PopUpCal.prototype, {
	...
	_connectCalendar: function(target, inst) {
		...
		if (appendText) {
			jQueryinput.after('<span class="calendar_append">' + appendText + '</span>');
		}
		...
		if (autoPopUp == 'button' || autoPopUp == 'both') { // pop-up calendar when button clicked
			...
			var trigger = jQuery(buttonImageOnly ? '<img class="calendar_trigger" src="' +
				...
				buttonText) + '</button>');
			jQueryinput.wrap('<span class="calendar_wrap"></span>').after(trigger);
			trigger.click(this.showFor);
		}
		jQueryinput.keydown(this._doKeyDown).keypress(this._doKeyPress);
		jQueryinput[0]._calId = inst._id;
	},
